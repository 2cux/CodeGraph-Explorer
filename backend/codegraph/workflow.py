...
def run_explain(
    ...
    # At this point, symbol is guaranteed to be a non-empty string
    ...
    if include_snippet and project_root and node.file_path:
        ...
        source_path = (Path(project_root) / node.file_path).resolve()
        # Security: ensure the resolved path is within the project root
        resolved_root = Path(project_root).resolve()
        try:
            source_path.relative_to(resolved_root)
        except ValueError:
            source_snippet = {"included": False, "reason": "path_traversal_blocked"}
        else:
