...
function NodeMatrix() {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: `120px repeat(${states.length}, 1fr)`, gap: 8, alignItems: "center" }}>
      ...
      {kinds.map((k) => (
        <>
          <div key={`${k}-label`} className="cg-mono" style={{ fontSize: 11, color: "var(--cg-text-secondary)" }}>{k}</div>
          {states.map((s) => (
            <div key={`${k}-${s}`} style={{ padding: 6, position: "relative", minHeight: 64 }}>
              <GraphNodeView
                standalone
                node={{
                  id: `${k}-${s}`, x: 0, y: 0, kind: k,
                  name: sampleName(k), path: samplePath(k),
                  confidence: 0.84, state: s,
                }}
              />
            </div>
          ))}
...
function EdgeMatrix() {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: `120px repeat(${labels.length}, 1fr)`, gap: 8, alignItems: "center" }}>
      <div />
      {labels.map((l) => (
      ...
      {states.map((s) => (
        <>
          <div key={`${s}-label`} className="cg-mono" style={{ fontSize: 11, color: "var(--cg-text-secondary)" }}>{s}</div>
          {labels.map((l) => (
            <EdgeSample key={`${s}-${l}`} label={l} state={s} />
          ))}
        </>
      ))}
    </div>
