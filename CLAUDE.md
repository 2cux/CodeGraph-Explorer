...
核心定位：**MCP-first**。MCP 细粒度图查询工具是 Agent 的主入口，Evidence Pack 是可选的非 MCP 快照。
...
0. **Code Review** — 任何代码修改后，先调用 `/code-review` skill 对当前 diff 进行审查。
1. **构建检查** — 后端：`pip install -e backend[mcp,watch]` 无报错。
2. **测试** — `pytest backend/tests/` 通过。
