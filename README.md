...
Configures Claude Code and Cursor to start CodeGraph automatically.
...
Builds the local `.codegraph/` index and installs a post-commit auto-update hook by default.
...
Verifies the index, MCP configuration, auto-update hook, and local environment.

After this, open Claude Code or Cursor and ask it to use CodeGraph for codebase navigation.
...
Or remove the post-commit hook:
...
MCP configuration files (`~/.claude.json`, `~/.cursor/mcp.json`) are not removed automatically. To clean them up:
